...
function oncostBtnElClicked() {
  ...
  document.getElementById(
    "carRentalBaseCost"
  ).innerHTML = ` $${baseCarRentalCost.toFixed(2)}`;
  ...
  document.getElementById(
    "optionsCost"
  ).innerHTML = ` $${totalOptionsCost.toFixed(2)}`;
  ...
  document.getElementById("ageSurcharge").innerHTML = ` $${surcharge.toFixed(
    2
  )}`;

  //calc Total
  const totalCost = surcharge + totalOptionsCost + baseCarRentalCost;
  document.getElementById("totalRentalCost").innerHTML = ` $${totalCost.toFixed(
    2
  )}`;
